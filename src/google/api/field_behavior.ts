...
export const fieldBehavior: Extension<FieldBehavior[]> = {
  number: 1052,
  tag: 8418,
  singularTag: 8416,
  repeated: true,
  packed: false,
  encode: (value: FieldBehavior[]): Uint8Array[] => {
  ...
  decode: (tag: number, input: Uint8Array[]): FieldBehavior[] => {
    const values: FieldBehavior[] = [];
    for (const buffer of input) {
      const reader = _m0.Reader.create(buffer);
      if (tag == 8418) {
        const end2 = reader.uint32() + reader.pos;
        while (reader.pos < end2) {
          values.push(reader.int32() as any);
        }
      } else {
        values.push(reader.int32() as any);
      }
